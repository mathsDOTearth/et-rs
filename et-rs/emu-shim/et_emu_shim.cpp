...
et_emu_dev *et_emu_create(const char *sdk_prefix, const char *run_dir,
  ...
  try {
    ...
    opt.startGdb = false;
    // Leave the emulator's memory-consistency checkers at their defaults (on):
    // they model the ET-SoC-1's coherence rules, so a demo that passes with them
    // enabled is coherence-correct for real hardware.
